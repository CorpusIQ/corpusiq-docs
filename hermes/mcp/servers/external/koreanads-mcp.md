...
```bash
# Check koreanads.com for MCP endpoint details
# Expected pattern (verify on site):
hermes mcp add koreanads --url https://koreanads.com/mcp
```

**Prerequisites:** Check koreanads.com for account requirements and API access.

## Key Tools (Expected)

| Tool | Description |
|------|-------------|
| `search_plans` | Search advertising plans by channel, budget, audience |
| `get_plan_details` | Full details for a specific advertising plan |
| `compare_plans` | Side-by-side comparison of multiple plans |
| `get_market_insights` | Korean advertising market trends and data |
| `estimate_reach` | Estimated reach and impressions for a plan |
