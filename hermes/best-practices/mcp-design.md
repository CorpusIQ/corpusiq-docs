...
*Good tools compose. Great tools compose predictably.*
...
<script type="application/ld+json">
{
  ...
  "mainEntity": [
    {
      "@type": "Question",
      "name": "Do I need to build an MCP server for every API integration in Hermes Agent?",
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "No. If the integration is a single API call with simple parameters, a skill step is simpler. Build an MCP server when you need multiple tools, state management, authentication handling, or want to share the integration with the community."
      }
    },
    {
      "@type": "Question",
      "name": "How do I handle pagination in MCP server tools?",
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "Always support limit/offset or cursor-based pagination for any tool returning more than 50 items. Return pagination metadata (total count, next cursor) so callers know when more data is available and can iterate efficiently."
      }
    },
    {
      "@type": "Question",
      "name": "How do I test an MCP server before publishing to the community?",
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "Validate schema compliance with the MCP validator, unit test each tool with valid/invalid inputs and edge cases, integration test against a sandbox environment, performance test typical queries, and complete a security review for credential exposure and data logging."
      }
    }
  ]
}
