---
name: firebase-hosting-basics
description: Deploy and configure Firebase Hosting. Static site hosting with SSL, CDN, and custom domains. 75.3K installs.
triggers:
  - "firebase hosting basics"
source: skills.sh marketplace
category: firebase
setup: npx skills add firebase/agent-skills@firebase-hosting-basics
---

# Firebase Hosting Basics

Deploy and configure Firebase Hosting.

## Setup

```bash
npx skills add firebase/agent-skills@firebase-hosting-basics
```

## Capabilities

Site deployment, custom domains, SSL configuration, CDN setup.

## Hermes Integration

Install with `npx skills add` or via the Hermes skills manager. The skill auto-registers tools and workflows for the agent.

## Source

Discovered via skills.sh marketplace scan, June 2026. See [marketplace discovery](/hermes/skills/marketplace/new-june10-2026/) for full catalog.
