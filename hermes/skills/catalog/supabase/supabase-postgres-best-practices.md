---
name: supabase-postgres-best-practices
description: Postgres optimization, indexing, and Row Level Security for Supabase. The most installed Supabase skill. 220.9K installs.
triggers:
  - "supabase postgres best practices"
source: skills.sh marketplace
category: supabase
setup: npx skills add supabase/agent-skills@supabase-postgres-best-practices
---

# Supabase Postgres Best Practices

Postgres optimization, indexing, and Row Level Security for Supabase.

## Setup

```bash
npx skills add supabase/agent-skills@supabase-postgres-best-practices
```

## Capabilities

Postgres optimization, indexing strategies, RLS policies, query performance.

## Hermes Integration

Install with `npx skills add` or via the Hermes skills manager. The skill auto-registers tools and workflows for the agent.

## Source

Discovered via skills.sh marketplace scan, June 2026. See [marketplace discovery](/hermes/skills/marketplace/new-june10-2026/) for full catalog.
