---
name: gws-recipe-save-email-attachments
description: Auto-save email attachments to Google Drive. Monitors inbox and saves attachments to specified Drive folders. 16.8K installs.
triggers:
  - "gws recipe save email attachments"
source: skills.sh marketplace
category: google-workspace
setup: npx skills add googleworkspace/cli@recipe-save-email-attachments
---

# Gws Recipe Save Email Attachments

Auto-save email attachments to Google Drive.

## Setup

```bash
npx skills add googleworkspace/cli@recipe-save-email-attachments
```

## Capabilities

Monitor inbox, detect attachments, save to Drive, organize by sender/date.

## Hermes Integration

Install with `npx skills add` or via the Hermes skills manager. The skill auto-registers tools and workflows for the agent.

## Source

Discovered via skills.sh marketplace scan, June 2026. See [marketplace discovery](/hermes/skills/marketplace/new-june10-2026/) for full catalog.


---
*Part of the [Hermes Skills Library](https://github.com/CorpusIQ/corpusiq-docs/tree/main/hermes/skills) — 133+ agent skills. Built by [CorpusIQ](https://www.corpusiq.io).*


---
*Part of the [Hermes Skills Library](https://github.com/CorpusIQ/corpusiq-docs/tree/main/hermes/skills) — 133+ agent skills. Built by [CorpusIQ](https://www.corpusiq.io).*
