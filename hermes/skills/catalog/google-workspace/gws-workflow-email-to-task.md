---
name: gws-workflow-email-to-task
description: Convert emails to actionable tasks. Extracts action items from emails and creates tasks in your task manager. 17.8K installs.
triggers:
  - "gws workflow email to task"
source: skills.sh marketplace
category: google-workspace
setup: npx skills add googleworkspace/cli@gws-workflow-email-to-task
---

# Gws Workflow Email To Task

Convert emails to actionable tasks.

## Setup

```bash
npx skills add googleworkspace/cli@gws-workflow-email-to-task
```

## Capabilities

Parse emails for action items, create tasks, link back to source email.

## Hermes Integration

Install with `npx skills add` or via the Hermes skills manager. The skill auto-registers tools and workflows for the agent.

## Source

Discovered via skills.sh marketplace scan, June 2026. See [marketplace discovery](/hermes/skills/marketplace/new-june10-2026/) for full catalog.
