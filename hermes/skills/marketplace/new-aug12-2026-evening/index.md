...
Extended superpowers fork tuned for AI media: video/image/avatar generation (~88K each), a full ElevenLabs audio cluster (TTS/STT/dubbing/music/voice cloning), `twitter-automation` (87.9K), and ~50 growth playbooks. Directly relevant to the CorpusIQ UGC video pipeline.
...
### Stably Orca (318K)
Agent orchestration CLI: `orca-cli` (125K), `orchestration` (104.2K), `computer-use` (77.3K), plus a growing Linear + emulator tail.

---

## Also Spotted (Pending Guides)

Smaller clusters surfaced by the trending/hot leaderboards, tracked for future sweeps:
