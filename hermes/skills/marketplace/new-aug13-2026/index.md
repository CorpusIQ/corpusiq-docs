...
- **INSTALL:** `extract-design-system` (design quality), `flowkit reddit-automation` (community), `humanlayer show-me` (governance patterns)
- **EVALUATE:** App Store Connect CLI cluster (if mobile deployments enter scope), GenMedia model pins (per-backend API costs)
- **MONITOR:** Pika Plugins (needs Pika backend), NuShell Pro, Fetcher scrapers

## Source

skills.sh trending (24h) and hot (1h) leaderboards via the skills-monitor cron. All install counts as listed on skills.sh at sweep time.
