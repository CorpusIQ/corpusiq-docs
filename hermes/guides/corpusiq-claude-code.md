...
3. Each connection uses read-only OAuth — no write access, no data stored
...
Read-only OAuth on every connection. Zero data stored. Every answer cites its exact source.
