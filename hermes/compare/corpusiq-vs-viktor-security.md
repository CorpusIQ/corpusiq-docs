...
External-source retrieval tools are read-only; write-capable and control-plane tools are separately named and annotated. An assistant that only reads cannot accidentally delete a product, change a price, or send an email. Validation happens before answers, not after damage.
